...
但不能用 list。STL里面默认用的是vector），Functional 就是比较的方式，当需要用自定义的数据类型时才需要传入这三个参数，
使用基本数据类型时，只需要传入数据类型，默认是大顶堆

默认是大顶堆
priority_queue<int, vector<int>, greater<int> > c;  //这样就是小顶堆 (和map等不一样)
...
void priority_queue_()
{
	...
	priority_queue<pair<int, int>, vector<pair<int, int>>, CmpByValueLess> q;
