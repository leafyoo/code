...
{
	assert(a && len);			//还是加上空指针判断吧
	for(int i = 1, j, tmp; i < len; ++i)//注意j必须要在这里定义!!因为在下面的for循环之外有用到
	{
		tmp = a[i];							//下面,tmp < a[j],如何确定是<，就看什么条件下可以继续执行循环，例数组:1，2
		for(j = i-1; j>=0 && tmp < a[j]; --j)//tmp < a[j]放在这里和在下面用if(tmp < a[j])，效果不同，这里可以让循环提前终止!!!不必浪费时间
		{
			a[j+1] = a[j];
		}
...
{
	for(int i = gap, tmp, j; i < len; ++i)//这里++i不变,不然岂不漏掉一些元素了。
	{
		tmp = a[i];
...
void BubbleSort_2(int *a, int len)
{
	...
	while(chg > 0)
	{
		len = chg;
		chg = 0;
		//原生态的冒泡排序中，外层for循环起到改变下面的len的作用，注意体会
		for(j = 0; j < len-1; ++j)
		{
			if(a[j] > a[j+1])
			{
				swap(&a[j], &a[j+1]);
				//这里change = j;相当于将下一次while循环里的 for循环中的len减1(或更多)，
...
int part(int *a, int b, int e)
{
	...
	while(b < e)
	{
		while(b < e && a[e] >= tmp)//无忽略"="，是:>=，//a[e] >= tmp,如何确定是<，就看什么条件下可以继续执行循环，显然是
			--e;
	...
	//基数归位
	a[b] = tmp;			//因为最开始处储存了a[low]，提取出来了，所以这里要还原给通过循环实际找到的low
	return b;
}
void qSort(int *a, int b, int e)
{
	if(b >= e)		//因为low >= high的话，下面依然会执行一系列操作，没用的!当然要禁止
		return;		//所以无需参与排序
...
void shift(int *a, int b, int e)
{
	...
	a[b] = tmp;          //给头替换为找到的最大值,这里不是Z字走位，因为上面 b = i;
}
void HeapSort(int *a, int len)
{
	//心里有堆即有堆，下面进行堆的初始化，即:调整堆
	//由堆的性质:a[len/2 - 1]为堆的最后一个非叶子节点
...
void doMsort(int *a, int *tmp, int b, int e)
{
	if(b < e)
	{
		...
		doMsort(a, tmp, b, m);	//这里是m，因为除法只保留整数，可以举例，如4个元素
		doMsort(a, tmp, m+1, e);
...
{
	int *tmp = malloc(len * sizeof(a[0]));	//所以,空间复杂度O(N)
