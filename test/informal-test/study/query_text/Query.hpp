...
class Query_base
{
		friend class Query(); //将友元放在类开始的地方
	protected:
		typedef TextQuery::line_no line_no;
		virtual ~Query_base(){}
	private:
		virtual set<line_no> eval (const Text_query &) const = 0;
		virtual ostream & display (ostream & = cout) const = 0;
		
};//为防止遗漏，编码时一次性键入 class{ };
...
{
	return new AndQuery(lsh, rhs);
}


inline Query operator|(const Query &lhs, const Query &rhs)
{
	return new OrQuery(lsh, rhs);
}
